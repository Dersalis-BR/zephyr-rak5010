...
{
	int parsed_len = 0;
	u8_t *argv[MODEM_CMD_HANDLER_MAX_PARAM_COUNT];
	u16_t argc = 0U;
	...
	/* call notify handler */
	/*
	if (data->notify_handler && data->notify_any_data) {
		data->notify_handler(data, match_len - cmd->cmd_len - parsed_len,
			  argv, argc);
	}
    */
	/* call handler */
...
static u8_t response_decode(char* rx_cmd){

	if(rx_cmd[0] != '+') return UNKNON_HANDLER;

	if((rx_cmd[1] == 'Q')&&(rx_cmd[2] == 'M')&&(rx_cmd[3] == 'T'))
		return MQTT_HANDLER;	

	return UNKNON_HANDLER;
}
				...
				struct modem_iface *iface)
{
	...
	int ret;
	u16_t offset, len;
	char rx_cmd[MODEM_CMD_HANDLER_MAX_SIZE];
	...
	while (true) {
		...
		if (rx_len < bytes_read) {
			printk("Data was lost! read %zu of %zu!\n",
				rx_len, bytes_read);
		}
	}
	...
	while (data->rx_buf) {
		...
		if (data->match_buf_len < match_len) {
			printk("Match buffer size (%zu) is too small for "
		...
		//printk("----> [%s] (len:%u)\n",data->match_buf, match_len);
		
		cmd = find_cmd_match(data);
		if (cmd) {
			
			printk("match cmd [%s] (len:%u)\n",cmd->cmd, match_len);
		...
		else if (data->notify_any_data){
			memset(&rx_cmd,0,sizeof(rx_cmd));
			strncpy(rx_cmd,data->match_buf,match_len);

			u8_t type = response_decode(rx_cmd);			

			if((type == MQTT_HANDLER) && data->mqtt_handler)
			 data->mqtt_handler(rx_cmd,match_len);
			   ...
			   bool no_tx_lock)
{
	...
	if (ret < 0) {
		printk("foi direto pro exit com o return carrier\n");
		goto exit;
	}
...
static int _modem_cmd_send_no_r(struct modem_iface *iface,
			   struct modem_cmd_handler *handler,
			   struct modem_cmd *handler_cmds,
			   size_t handler_cmds_len,
			   const u8_t *buf, struct k_sem *sem, int timeout,
			   bool no_tx_lock)
{
	...
	ret = modem_cmd_handler_update_cmds(data, handler_cmds,
					    handler_cmds_len, true);
	if (ret < 0) {
		printk("foi direto pro exit sem o return carrier\n");
		goto exit;
	}
...
int modem_cmd_send_no_r(struct modem_iface *iface,
		   struct modem_cmd_handler *handler,
		   struct modem_cmd *handler_cmds, size_t handler_cmds_len,
		   const u8_t *buf, struct k_sem *sem, int timeout)
{
	return _modem_cmd_send_no_r(iface, handler, handler_cmds, handler_cmds_len,
			       buf, sem, timeout, false);
}
